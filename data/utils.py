...
# noisify_pairflip call the function "multiclass_noisify"
def noisify_asymmetric(y_train, noise, random_state=None, nb_classes=10):
    """mistakes:
        flip in the pair
    """
    # P = np.eye(nb_classes)
    ...
    if n > 0.0:
        # 0 -> 1
        P = np.eye(nb_classes) * (1 - n)
        row_indices = np.arange(nb_classes)
        for i in range(nb_classes):
            P[i][np.random.choice(row_indices[row_indices != i])] = n
...
def noisify_tridiagonal(y_train, noise, random_state=None, nb_classes=10):
    ...
    if n > 0.0:
        ...
        for i in range(1, nb_classes-1):
            P[i, i-1], P[i, i], P[i, i + 1] = n/2, 1. - n, n/2
        P[nb_classes-1, nb_classes-2], P[nb_classes-1, nb_classes-1], P[nb_classes-1, 0] = n/2, 1. - n, n/2
...
def noisify(dataset='mnist', nb_classes=10, train_labels=None, noise_type=None, noise_rate=0, random_state=0):
    if noise_type == 'pairflip':
        ...
        # print(len(set(train_noisy_labels) & set(train_labels)))
        print('ok')
    ...
    if noise_type == 'tridiagonal':
        train_noisy_labels, actual_noise_rate = noisify_tridiagonal(train_labels, noise_rate, random_state=0, nb_classes=nb_classes)
    if noise_type == 'asymmetric':
        train_noisy_labels, actual_noise_rate = noisify_asymmetric(train_labels, noise_rate, random_state=0,
