...
from PreResNet_cifar import resnet34
from data.cifar_aug_one import CIFAR10, CIFAR100
from data.mnist import MNIST
from data.clothing1M_dataloader import clothing_dataloader
...
parser.add_argument('--noise_type', type=str, help='[pairflip, symmetric,tridiagonal,instance]', default='instance')
...
def train(train_loader, soft_labels, epoch, model1, optimizer1,feature):
    ...
    feature_labels = torch.zeros(noisy_label.shape[0], 100, dtype=torch.float).cuda(non_blocking=True)
    ...
    if epoch < args.es:
        ...
        centers = torch.zeros((100, 128))
        for cls_ in range(100):
        ...
        # overall_distance = np.genfromtxt(filling_values=0)
        # overall_distance=np.nan_to_num(overall_distance)
    ...
    for i, (images, labels, indexes) in enumerate(train_loader):
        ...
        if epoch < args.es:
            ...
            soft_labels[ind] = 0.8 * soft_labels[ind] + 0.199 * prob_1 + 0.001 * feature_labels[ind]
...
def main():
    ...
    if args.dataset == 'cifar100':
        ...
        soft_labels = torch.zeros(noisy_label.shape[0], 10, dtype=torch.float).cuda(non_blocking=True)
        soft_labels[torch.arange(noisy_label.shape[0]), noisy_label] = 1



    print('building model...')
    cnn1=resnet34(num_classes=100)
